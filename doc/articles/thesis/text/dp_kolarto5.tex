...
to~guarantee specifications fulfillment
in~complex systems.
...
K~řešení \acr{ode} interně používá nástroj
...
% Wed 09 May 2018 02:11:19 AM CEST
...
% Wed 09 May 2018 02:44:36 AM CEST
...
Projekt jsem nazval \name{\acrf{sos}}.
Řešič je koncipován
jako soubor knihoven, appletů a hlavních aplikací,
implementovaných v~jazyce C++.
...
Moduly, které zprostředkovávají
některý z~\acr{smt} či \acr{ode} řešičů,
...
% Tue 08 May 2018 06:16:47 PM CEST
...
\pic{expr_tree.pdf}{expr:tree}{0.65}
   {Ukázka struktury objektu třídy \id{Expr}}
   {Obrázek zanedbává informaci o~tom, že každý potomek \id{Expr}
    je uložen jako ukazatel na~abstraktní třídu \id{Expr\_\-place}.
    Červené šípky ukazují na~počátečního potomka.
    \id{pos} značí interní ukazatel na~aktuální pozici
    v~rámci sekvenčního průchodu potomky každého objektu \id{Expr}.}

Struktura objektu třídy \id{Expr}
je ilustrována na~obrázku \rfPic{expr:tree}.
...
% Tue 08 May 2018 06:19:22 PM CEST
...
% Tue 08 May 2018 06:40:42 PM CEST
...
\tab{const_ids_rows}{0.75}
   {Struktura \id{Const\_\-ids\_\-rows} pro~\acr{ode} s~klíčem \id{x}}
   {Názvy identifikátorů jsou jen příklady (nicméně mají typický tvar),
    smí být libovolné.
    V~tomto případě obsahuje model dva nezávislé systémy
    používající definici \acr{ode} \id{x}.
    Každá \acr{ode} obsahuje vlastní takovou tabulku.}
   {r||l|l|l}
   {\id{Time\_\-const\_\-ids} & \id{Const\_\-ids\_\-entry} $1$
                              & \id{Const\_\-ids\_\-entry} $2$
                              & \dots\\\hline\hline
    \id{t\_0}, \id{t\_1} & \id{dx.1\_0}, \id{x.1\_0}, \id{k\_0}
                         & \id{dx.2\_0}, \id{x.2\_0}, \id{k\_0}
                         & \dots\\\hline
    \id{t\_1}, \id{t\_2} & \id{dx.1\_1}, \id{x.1\_1}, \id{k\_1}
                         & \id{dx.2\_1}, \id{x.2\_1}, \id{k\_1}
                         & \dots\\\hline
    \dots
   }

Struktura \id{Const\_\-ids\_\-rows} pro~jednu \acr{ode}
je naznačena v~tabulce \rfTab{const_ids_rows}.
...
\pic{odes_spec.pdf}{odes:spec}{0.75}
   {Struktura a vztah objektů \id{Odes\_\-spec} a \id{Param\_\-keyss}}
   {Specifikace jsou v~nezávislém tvaru.
    Tvar \id{Param\_\-keys} pro~sjednocený by odpovídal \id{[x, y, t]}.}

Strukturu a vztah konstruktů
\id{Odes\_\-spec} a \id{Param\_\-keyss}
zachycuje obrázek \rfPic{odes:spec}.
...
% Wed 09 May 2018 03:28:31 AM CEST
...
% Tue 08 May 2018 11:58:54 PM CEST
...
Pro~některé volby hodnot \id{T} není možné invarianty splnit.
...
% Wed 09 May 2018 12:18:50 AM CEST
...
% Tue 08 May 2018 11:08:40 PM CEST
\chapter{Další úlohy}\label{appendix:next_examples}
Zde uvádím úlohu, kterou se mi nepodařilo srovnat
s~řešičem dReal.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tue 08 May 2018 11:10:10 PM CEST
\begin{section}{Uzavřená funkce}\label{s:next_examples:fun}
Tato modelová úloha má zcela
nedeterministické přechody mezi diskrétními stavy
a slouží pro~ověření zacházení s~návraty.

Jsou dány dvě schodovité funkce
jako horní (\id{high}) a dolní (\id{low}) mez:
\begin{flalign*}
   \der{high} &= t - high \cdot \sin (high) \\
   \der{low}  &= \frac{t}{K} - low \cdot \sin (low \cdot K)
\end{flalign*}
kde jsem v~tomto případě zvolil $K = 2{,}5$.

Dále je dána funkce \id{y} s~variantami derivací
$\der{y} = y$ a $\der{y} = -y$.
Úkolem je zaručit invariant \id{low <= y <= high},
přičemž je nutné vždy integrovat po~fixně danou dobu \id{T}
(to odpovídá fixní délce fáze našeho řešiče).

Pokoušel jsem se úlohu modelovat v~dReal,
ale nebyl jsem schopen dosáhnout dokončení výpočtu
ani do~úseku \id{t = 5}
(do~té doby je úloha nezajímavá,
protože např. téměř stačí jen setrvat na~hodnotě $0$).
Konkrétně výpočet selhal už ve~stádiu generování
vstupu nástrojem dReach,
který současně s~generováním ověřuje splnitelnost
zvolené cesty (přechody mezi diskrétními stavy).
Počíná si však dle mého názoru velmi neefektivně:
postupně zkouší všechny možné
kombinace \hl{nezávisle} na~sobě,
dokud nenarazí na~splnitelnou.
Pro~\id{t = 5} a rozumně malé \id{T} (např. $0{,}25$;
jinak není možné invarianty dodržet)
to dělá cestu o~$20$ přechodech,
což je $2^{20}$ kombinací.
Po~dvou a půl hodinách se dReach nedostal
ani do~$400$.~kombinace (a všechny byly do té doby nesplnitelné).

\tab{fun_cmp}{0.6}
   {Uzavřená funkce: srovnání výstupů}
   {\id{T} značí fixní délku fáze.
    Pro~\id{T = 0.2} program po~delší době spadl
    (pravděpodobně přetekl systémový zásobník).}
   {r||c|l}
   {\id{T} [s] & Délka výpočtu [s] & Výstup \\ \hline
    $0{,}2$   & X \ \          & X \\ \hline
    $0{,}25$  & $4{,}8$ \ \    & \id{sat} \\ \hline
    $0{,}33$  & $1{,}3$ \ \    & \id{sat} \\ \hline
    $0{,}4$   & $0{,}73$ \     & \id{sat} \\ \hline
    $0{,}5$   & $34$ \ \ \ \ \ \ & \id{unsat} \\ \hline
    }

\pic{fun_0_25.pdf}{fun:0.25}{0.9}
   {Uzavřená funkce \id{y} s~délkou fáze $0{,}25$ s.}
   {}
\pic{fun_0_4.pdf}{fun:0.4}{0.9}
   {Uzavřená funkce \id{y} s~délkou fáze $0{,}4$ s.}
   {}

Výpočet jsem provedl až~do~\id{t = 30}.
V~tabulce \rfTab{fun_cmp} jsou zaznamenány naměřené výsledky
a na~obrázcích \rfPic{fun:0.25} a \rfPic{fun:0.4} výsledné podoby řešení.
\end{section} %%Uzavřená funkce

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tue 08 May 2018 09:02:21 PM CEST
...
% Tue 08 May 2018 09:05:07 PM CEST
...
Ideální je zvolit hodnotu \id{\#T} co nejvyšší,
aby byly co nejnižší nároky na~řídící systém,
ale v~první řadě musí být splněny invarianty.
Musíme ručně experimentovat s~různými hodnotami.
\end{section} %%Tvar vstupu

%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tue 08 May 2018 09:50:11 PM CEST
\begin{section}{Výsledky}\label{s:example:results}
Experimentovali jsme s~různými hodnotami pro~\id{\#T}
a ověřili, pro které můžeme zadané specifikace zaručit.
Po~sestavení vstupu už to byla rychlá práce.

\tab{thermostat_cmp}{0.6}
   {Termostat: srovnání výstupů}
   {\id{T} značí fixní délku fáze.
    (V~tomto případě nás příliš doba výpočtu nezajímá.)}
   {r||c|l}
   {\id{T} [s] & Délka výpočtu [s] & Výstup \\ \hline
    $0{,}25$  & $1{,}25$ \ \ \ & \id{sat} \\ \hline
    $0{,}33$  & $0{,}75$ \ \ \ & \id{sat} \\ \hline
    $0{,}4$   & $0{,}52$ \ \ \ & \id{sat} \\ \hline
    $0{,}5$   & $0{,}20$ \ \ \ & \id{unsat} \\ \hline
    }

Do~tabulky \rfTab{thermostat_cmp} jsme zaznamenali naměřené výsledky.
Pro~\id{T = 0.5} jsme zjistili, že již specifikace není možné zaručit.

\pic{thermostat_0_25.pdf}{thermostat:0.25}{0.9}
   {Termostat s~délkou fáze $0{,}25$ s.}
   {Veličina \id{x} značí teplotu.}

\pic{thermostat_0_4.pdf}{thermostat:0.4}{0.9}
   {Termostat s~délkou fáze $0{,}4$ s.}
   {Veličina \id{x} značí teplotu.
    Meze pro~\id{x} byly splněny velmi těsně.}

Také jsme si na~obrázky
\rfPic{thermostat:0.25} a \rfPic{thermostat:0.4}
vykreslili podobu možného řešení.
Pro \id{T = 0.4} jsou specifikace splněny těsně,
a je na~našem zvážení, zda raději nezvolit
nižší hodnotu, aby zbyla nějaká rezerva.

Tím jsme úspěšně ověřili specifikaci našeho modelu termostatu
a můžeme se nyní pustit do~fyzické realizace,
a umožnit tak automatizované řízení naší části montážní linky.

%%%

% Tue 08 May 2018 09:50:11 PM CEST
\begin{paragraph}{Srovnání s~dReal.}\label{s:example:results:cmp}
Tato úloha je opět orientována na~čas,
takže se dalo očekávat, že si povede řešič dReal hůře než náš.
Navíc je přímo nutné zvolit konkrétní fixní délku periody \id{T}.
Zvolil jsem tu nejjednodušší variantu,
kterou jsem použil v~měření vlastního řešiče: \id{T = 0.4},
a použil jsem aproximované intervaly,
stejně jako v~našem zadání.
Tuto úlohu řešil dReal $46{,}3$ sekund
a vyžadoval více než~$2$ GB paměti.
To je výrazně horší výsledek než náš.

\pic{thermostat_dreal_x.pdf}{thermostat:dreal:x}{0.65}
   {Termostat v~dReal s~délkou fáze $0{,}4$}
   {Veličina \id{x} značí teplotu.
    (Délka fáze odpovídá fixní periodě,
    která je v~této úloze explicitně zadána.)}

Výsledná trajektorie z~řešiče dReal
je zachycena na~obrázku \rfPic{thermostat:dreal:x}.

Navíc jsem měl opět podobné problémy s~nástrojem dReach,
jaké jsem popisoval v~sekci \rf{s:next_examples:fun}.
Tentokrát se počet kroků \acr{bmc} rovnal $150$
a dReach ani nebyl schopen spustit první kombinaci,
protože předtím generuje všechny možné kombinace
(tj.~$2^{150}$)
do~souboru a vyčerpal paměť.
Lze mu však explicitně nastavit konkrétní kombinaci,
po~které se má vydat,
a zadal jsem mu sekvenci,
která jen cyklicky přecházela z~jednoho stavu do~druhého,
a tato cesta byla (naštěstí) splnitelná.
\end{paragraph} %%Srovnání s~dReal.

%%%

\end{section} %%Výsledky
