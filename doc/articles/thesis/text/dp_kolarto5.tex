...
% \newcommand{\glosItem}[2]{\item[\itab{#1}]\tab{#2}}
% \newcommand{\glosSymb}[2]{\glosItem{\ensuremath{#1}}{#2}}
% \newcommand{\glosOpSymb}[2]{\glosSymb{#1{A},#1{a}}{#2 $A,a$}}
% \newcommand{\glosBinOpSymb}[2]{\glosSymb{A#1B}{#2 $A$ a $B$}}
% \glosOpSymb{\vec}{Vektor}
% \glosOpSymb{\mat}{Matice}
% \glosOpSymb{\pred}{Predikovaná hodnota}
% \glosOpSymb{\gues}{Průběžně odhadovaná hodnota}
% \glosOpSymb{\rand}{Náhodná proměnná}
% \glosOpSymb{\mean}{Očekávaná hodnota náhodné proměnné}
% \glosOpSymb{\trans}{Transpozice}
% \glosOpSymb{\conj}{Konjugace}
% \glosOpSymb{\ctrans}{Konjugovaná transpozice}
% \glosOpSymb{\inv}{Inverze}
% \glosOpSymb{\norm}{Eukleidovská vektorová norma}
% \glosBinOpSymb{*}{Konvoluce mezi}
% \glosBinOpSymb{}{Násobení}
% \glosOpSymb{\id}{Identifikátor}
% \glosSymb{\zvec}{Nulový vektor}
% \glosSymb{\zmat}{Nulová matice}
% \glosSymb{\imat}{Jednotková matice}
...
\keywordsCS{\acr{sat}, \acr{smt}, numerické metody pro~\acr{ode},
analýza hybridních systémů}
\keywordsEN{\acr{sat}, \acr{smt}, numerical methods for~\acr{ode}s,
hybrid systems analysis}
% \website{http://site.example/thesis} %volitelná URL práce,
...
% Mon 03 Jul 2017 12:10:52 AM CEST
...
% Mon 03 Jul 2017 12:10:25 AM CEST
...
% Mon 16 Apr 2018 09:11:34 PM CEST
...
% Mon 23 Apr 2018 11:47:28 PM CEST
...
a \id{<token>} token.
První token výrazu odlišuje,
zda a o~jaký se jedná příkaz nebo funkci.
...
% Mon 16 Apr 2018 11:26:16 AM CEST
...
\id{+ \  - \  * \  / \  = \  < \  > \  <= \  >=
   \  not \  and \  or  \  =>}
\end{center}
\cmt{asociativity, chainability, ite, distinct}
\end{paragraph} %%Rezervované funkce.

%%%

% Tue 24 Apr 2018 12:26:14 PM CEST
...
teorie reálných čísel podle \acr{smt}-LIB:
\begin{itemize}
...
% Mon 16 Apr 2018 01:24:27 PM CEST
\begin{paragraph}{Nové rezervované funkce.}\label{p:design:spec:ilang:ode:reserved}
...
Přidány jsou následující funkce (resp. operátory):
\begin{center}
...
pro~účely propojení konstant druhu \id{Dt} dílčích kroků se~stavem systému.
...
% Tue 24 Apr 2018 12:35:29 AM CEST
...
% Sat 07 Apr 2018 07:50:08 PM CEST
...
není možné, aby se průběh stavu systému v~rámci jednoho kroku dynamicky měnil.
Jelikož je vstup statický,
je pro~modelování průběhu nutné využít mnoha konstant
o~předem známém počtu,
tzn.~že už při~modelování systému je nutné určit \hl{počet kroků simulace}%
\footnote{Pozor, jedná se o~jiné kroky než kroky \acr{ode} řešiče.}.
Rozložení kroků je určeno hodnotami konstant nezávislé proměnné \id{t},
mezi nimiž dochází k~integraci.
V~těchto krocích také dochází se skokům.
...
% Mon 16 Apr 2018 01:32:51 PM CEST
\begin{paragraph}{Deklarace a inicializace konstant.}
\label{p:design:spec:ilang:struct:const}
Všechny konstanty použité ve~všech krocích simulace
musejí být deklarovány a konstanty počátečního kroku
musí být i~definovány.
Každé konstantě je vhodné dát jako příponu číslo kroku,
kterého se týká.

Je vhodné deklarovat konstanty pro~jednotlivé kroky
nezávislé proměnné \id{t},
průběhů neznámých funkcí, diskrétních stavů
a konstant voleb variant derivací \id{dt}.
(Pozor na~konflikt identifikátorů konstant \id{dt}
...
se provádí příkazem \id{int\--ode}.
...
\end{verbatim}
\end{paragraph} %%Volba variant derivací

%%%

% Sat 07 Apr 2018 08:22:32 PM CEST
...
% Mon 23 Apr 2018 08:16:45 PM CEST
...
(např.~kroky simulace vstupního modelu),
...
% Mon 23 Apr 2018 09:07:20 PM CEST
...
ale naposledy definované lokální makro má vždy přednost.
...
je zavedeno rezervované makro s~názvem \id{\#}
(tj. volá se jako \id{\#\#}),
jehož obsahem je prázdný text.
...
% Mon 23 Apr 2018 09:52:52 PM CEST
...
% Mon 16 Apr 2018 09:14:44 PM CEST
...
% Tue 9 Apr 2018 20:40:26 AM CEST
...
% Tue 17 Apr 2018 11:23:55 AM CEST
...
bez~ohledu na konkrétní použitý řešič.
...
% Tue 9 Apr 2018 20:40:22 AM CEST
...
a celý zbytek vstupu poslat s~jen minimálními změnami
\acr{smt} řešiči jako inicializaci.
...
% Mon 09 Apr 2018 07:32:09 PM CEST
...
% Tue 10 Apr 2018 11:57:34 AM CEST
...
Každý výraz obsahuje pole potomků,
z~nichž každý je buď další podvýraz,
nebo element s~textovou hodnotou.
...
% Tue 10 Apr 2018 11:57:26 AM CEST
...
musí být jeho první prvek element s~názvem nějaké funkce,
...
\item je-li prvek element, provede se konverze na~datový typ;
   pokud konverze selže, je element považován za~nepřímý argument.
\end{itemize}
Nepřímé argumenty se později dosadí
jako parametry při~volání vyhodnocení
a textové hodnoty elementů jsou uloženy jako klíče argumentů.
...
% Tue 10 Apr 2018 11:57:53 AM CEST
...
s~různými vstupními argumenty, podle aktuálních hodnot
\acr{smt} konstant a kroku simulace.
...
(podle vzdálenosti mezi kroky simulace).
...
% Mon 16 Apr 2018 01:40:55 PM CEST
...
Postup je následující:
\begin{enumerate}
\item Nastavení \acr{smt} vstupu: zvolení logiky \id{QF\_\-UFNRA},
   definice druhu \id{Dt}, ad.
\item Zpracování definic diferenciálních rovnic:
   načtení definic diferenciálních rovnic do~výrazů
   a seznamů klíčů nepřímých argumentů pro~\acr{ode} řešič
   z~příkazů \id{define\--dt} a jejich substituce
   za~konstanty variant derivací druhu \id{Dt}.
\item Substituce příkazů integrací \id{int\--ode}
   za~pomocné konstanty nebo funkce druhu \id{Real};
   spárování vstupních konstant příkazů s~jednotlivými kroky simulace.
...
% Tue 17 Apr 2018 10:10:32 AM CEST
\chapter{Realizace}\label{ch:impl}
\cmt{komentář}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Tue 17 Apr 2018 10:10:59 AM CEST
...
a je veřejně dostupný na~adrese
...
% Tue 17 Apr 2018 10:56:38 AM CEST
...
\end{section} %%Implementace \acr{smt} řešiče
...
Realizaci specifikací diferenciálních rovnic
dynamicky z~textových řetězců
...
Nejprve popíši výrazy a jejich vyhodnocení,
poté abstraktní třídu \acr{ode} řešiče,
a konečně odvozené třídy s~konkrétními implementacemi řešení \acr{ode}.

%%%%%%%%%%%%%

% Tue 17 Apr 2018 03:36:08 PM CEST
\begin{subsection}{Realizace vyhodnocení výrazů}
\label{ss:impl:ode:exprs}
Výrazy reprezentuje třída \id{Expr}
a jejich vyhodnocení šablonová třída \id{Expr::\-Eval<Arg>}.

%%%

% Tue 17 Apr 2018 12:48:22 PM CEST
\begin{paragraph}{\id{Expr}}\label{p:impl:ode:exprs:expr}
je odvozená třída od~abstraktní třídy \id{Expr\_\-place}.
\id{Expr} využívá polymorfismu~---~obsahuje
stromovou strukturu jako pole ukazatelů na~\id{Expr\_\-place},
...
\begin{paragraph}{\id{Expr::\-Eval<Arg>}.}\label{p:impl:ode:exprs:eval}
...
pole klíčů nepřímých argumentů a pole jejich hodnot,
které se nastaví při~volání celého vyhodnocení jako funkce.
...
Také je možné je využít pomocí aplikace \id{eval},
která vyhodnocuje vstupní výrazy,
které mohou obsahovat i~nepřímé argumenty.
\end{subsection} %%Realizace vyhodnocení výrazů
...
% Wed 18 Apr 2018 10:53:27 AM CEST
\begin{paragraph}{Sjednocení klíčů}\label{p:impl:ode:solver:spec:unif}
Pokud je potřeba všechny \acr{ode} řešit současně
...
% Wed 18 Apr 2018 10:59:20 AM CEST
...
\begin{paragraph}{\id{Context}}\label{sss:impl:ode:solver:solve:ctx}
...
který je formátován stejným způsobem,
jako specifikace rovnic z~textu
(viz.~předchozí sekce),
a delegován na~konstrukci pomocí \id{Expr}.
Pokud je v~textu specifikován jen jeden \id{Context},
jsou rovnice řešeny sjednoceně.
\end{subsubsection} %%Řešení rovnic

%%%%%%%

% Wed 18 Apr 2018 11:54:13 AM CEST
...
\item přehledný výpis všech obsažených rovnic řešiče.
\end{itemize}
...
Nad~výchozím objektem této třídy se volá funkce \id{run}
s~argumenty z~příkazové řádky jako argumenty.
...
tak integraci všech rovnic současně,
...
\end{section} %%Implementace \acr{ode} řešiče

%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Mon 16 Apr 2018 10:17:50 PM CEST
\begin{section}{Implementace zpracování vstupu}\label{s:impl:parser}
\cmt{expr, mapy, makra}
